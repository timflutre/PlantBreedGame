...
log_info("Starting request processor")
  ...
  tryCatch(
    {
      ...
      if (file.exists(DATA_IN_USE_FILE)) {
        ...
        if (DATA_SESSION != last_session) {
          log_debug("Using session: {CURRENT_SESSION_ID}")
          last_session <- DATA_SESSION
        }
      } else {
        log_warn("missing `data_folder_in_use.txt`, retrying in 1s")
        Sys.sleep(1)
        next
      }
      ...
      log_info("Processing request [ID: {request_to_process$id}] from breeder '{request_to_process$breeder}': {request_to_process$name}")
    ...
    error = function(err) {
      log_error("REQUEST PROCESSOR FAILURE: {capture.output(print(err))}")
      Sys.sleep(1)
    }
  )
}
