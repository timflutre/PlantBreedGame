...
source("src/fun/func_pheno.R")
source("src/fun/func_permissions.R")
...
data_root <- normalizePath(Sys.getenv("PLANTBREEDGAME_DATA_ROOT", "./data"))
...
# create emtpy
options(DATA_DB = dbname)
db <- dbConnect(SQLite(), dbname = dbname)
dbDisconnect(db)

progress_detail <- paste("Data-Base initialisation DONE...")
progressBar$inc(
  amount = 1,
  detail = progress_detail
)
...
```{r table_constants}
progress_detail <- "Data-base: constants..."
progressBar$inc(
  amount = 1,
  detail = progress_detail
)
...
progress_detail <- "Data-base: game-sessions..."
progressBar$inc(
  amount = 1,
  detail = progress_detail
)
...
progress_detail <- "Data-base: breeders..."
progressBar$inc(
  amount = 1,
  detail = progress_detail
)
...
```{r fill_table}
progress_detail <- "Data-base: initial plant materials..."
progressBar$inc(
  amount = 1,
  detail = progress_detail
)
...
db_add_request(
  ...
  type = "pltmat",
  game_date = paste0(first.year - nb.years - 1, "-01-01")
)
...
db_update_request(id = initial_plant_mat_request_id, progress = 1)
...
system.time({
  haplo_id_association <- mclapply(1:length(ind.ids), function(i) {
    ...
    current_progress <- progressBar$getValue()
    current_progress_percent <- round((current_progress %% 1) * 100)
    progressBar$inc(
      amount = 1 / length(ind.ids),
      detail = paste("Save initial haplotypes:", current_progress_percent, "% ...")
    )
  }, mc.cores = nb.cores)
  haplo_id_association <- do.call(rbind, haplo_id_association)
  db_update_pltmat(haplo_id_association)
})

afs0 <- afs[thin.snps]
GV <- calculate_genetic_values(coll$genos, p0 = p0, afs0 = afs0)
...
progress_detail <- "Data-base: initial phenotypes..."
progressBar$inc(
  amount = 1,
  detail = progress_detail
)
...
progress_detail <- "Data-base: initial genotypes..."
progressBar$inc(
  amount = 1,
  detail = progress_detail
)
...
db_update_request(id = init_geno_req_id, progress = 1)
```
...
```{r save_afs_in_file}
progress_detail <- "Save allele frequencies..."
progressBar$inc(
  amount = 1,
  detail = progress_detail
)
...
Copy examples requests files:

```{r exemple_files}
file.copy("src/initial_examples_files/example_request_plant_material.txt", init.dir)
file.copy("src/initial_examples_files/example_request_request_data.txt", init.dir)
```



## Create initial "Admin" and "tests" breeders

```{r add_breeders}
progress_detail <- "Create initial breeders..."
progressBar$inc(
  amount = 1,
  detail = progress_detail
)
addNewBreeder("admin",
  "1234",
  permissions = sapply(PERMISSIONS_LIST, function(p) { # all premissions
    p$name
  }),
  data_truth = truth.dir,
  data_shared = shared.dir
)
addNewBreeder("test",
  "",
  permissions = c(
