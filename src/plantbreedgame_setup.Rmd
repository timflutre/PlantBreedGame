...
all.values <- paste(gsub("Coll", "Ind", ind.ids),
  ...
  ind.ids,
  rep(paste0(first.year - nb.years - 1, "-01-01 00:00:00"), length(ind.ids)),
  sep = "','", collapse = "'),('"
)
for (breeder in c(breeders, "init")) {
  tbl <- paste0("plant_material_", breeder)
...
system.time(
  tmp <- mclapply(1:length(ind.ids), function(i) {
    ...
    save(ind, file = f)
    for (breeder in breeders) {
      fto <- file.path(truth.dir, breeder, paste0(ind.id, "_haplos.RData"))
      if (!file.exists(fto)) {
        f_relative_path <- file.path("..", paste0(ind.id, "_haplos.RData"))
        file.symlink(f_relative_path, fto)
      }
    }
  }, mc.cores = nb.cores)
)
