...
output$evalUI <- renderUI({
  if (breederStatus()=="game master"){
    ...
    shinydashboard::box(width=12, title = "Choose an evaluation file:",
                        div( id="eval_file",
                             fileInput(inputId="file.eval",
                                       ...
                                       multiple=FALSE,
                                       accept=c(".txt", ".tsv")),
                             numericInput("nRep", "number of plot per individual:", 5, min = 1, max = 100),
                             actionButton("requestEval", "Lunch evaluation ! ")
                        )
    ),
    ...
    shinydashboard::box(width=12, title = "Content unavailable",
                                div(p("Sorry, this is only accessible for the Game-Master"))
    )
  }
...
output$evalGraphT1vT2<- renderPlotly({
  ...
  p <- plot_ly(data=dfPhenoPatho,
               ...
               color = ~breeder,
               text=~ind) %>%
    layout(title = "Trait 1 Vs Trait 2",
