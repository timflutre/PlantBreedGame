...
simulDat <- function(mu.0 = 40, sigma2 = 1, h2 = 0.75, sigma.a2 = NULL,
  ...
  Sigma <- matrix(
    c(
      var.midparents, covar.midpar.off,
  ...
  N <- I * J
  all.y <- MASS::mvrnorm(
    n = N, mu = c(mean.midparents, mean.offsprings),
    Sigma = Sigma
  )
...
applySelection <- function(mu.0, y, y.e, y.t = 43, sigma.02) {
  ...
  return(list(
    y.t = y.t, is.sel = is.sel, mu.s = mu.s, S = S, i = i, alpha = alpha,
    mu.1 = mu.1, R = R
  ))
}
...
plotRegMidparentsOffsprings <- function(mu.0, sigma.02, h2, y, y.e,
  ...
  arrows(
    x0 = mu.0 - 3 * sqrt(sigma.02),
    y0 = mu.0,
    x1 = mu.0 - 3 * sqrt(sigma.02),
    y1 = mu.1, lwd = 2, code = 3
  )
  text(
    x = mu.0 - 2.3 * sqrt(sigma.02),
    y = mu.0 + (R / 2),
    labels = paste0(
      "R = ", format(R, digits = 2),
