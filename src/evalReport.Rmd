...
```{r include=FALSE}
BVdta <- params$BVdta
BVdta$selected <- as.logical(BVdta$selected_for_evaluation) | as.logical(BVdta$control)
```
...
plot_ly(
  ...
  layout(
    title = paste0("Best individuals (Trait 1 x Trait 2)"),
    xaxis = list(
      title = "Generation",
      dtick = 1
    ),
...
myColors <- c(
  "#bbbbbb", "#165b16", "#da4a3b", "#0c246f", "#ffbe00",
...
for (breeder in unique(BVdta$breeder)) {
  ...
  generations <- sort(unique(dta$gen))

  for (i_gen in seq(unique(dta$gen))) {
    color_id <- ((i_gen - 1) %% length(myColors)) + 1
    color <- myColors[color_id]
    .gen <- generations[i_gen]
    for (.control in c(0, 1)) {
      for (.selected in c(0, 1)) {
        plt_dta <- dta[dta$gen == .gen &
          dta$control == .control &
          dta$selected_for_evaluation == .selected, ]
        if (nrow(plt_dta) == 0) {
          next
        }
        symbol <- NA
        ...
        if (.selected + .control >= 1) {
          line$width <- 1
        }

        name <- paste(
          "Generation", .gen,
          ifelse(.control == 1, "- Controls", ""),
          ifelse(.selected == 1, "- Selected", "")
        )
        plt <- plt %>% add_trace(
          data = plt_dta,
          x = ~trait1,
          ...
          hoverinfo = "text",
          text = ~ paste0(
            "\n<b>", name, ifelse(selected == 1, "*", ""), "</b>", # (in bold)
            "\nparent1: ", parent1_name,
            "\nparent2: ", parent2_name,
            "\nBV trait1 = ", round(trait1, 2),
            "\nBV trait2 = ", round(trait2, 2),
