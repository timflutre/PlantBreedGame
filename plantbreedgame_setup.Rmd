...
set.seed(1859) # better to change it from one game session to the next
```
...
if(Sys.info()["user"] == "tflutre"){
  root.dir <- "~/src/plantbreedgame/data"
...
cost.pheno.field <- 50
query <- paste0("INSERT INTO ", tbl, " VALUES",
                " ('cost.pheno.field', '", cost.pheno.field, "')") # in Mendels
res <- dbExecute(conn=db, query)
query <- paste0("INSERT INTO ", tbl, " VALUES",
...
query <- paste0("INSERT INTO ", tbl, " VALUES",
                " ('initialBudget', '", cost.pheno.field * nb.plots * 10 * 1.3, "')") # idem
res <- dbExecute(conn = db, query)
```
...
Make an example of an "evaluation" file:
```{r ex_eval}
dat <- data.frame(breeder=c("test", "test"),
                  ind=c("Coll0040", "Coll0007"))
f <- paste0(init.dir, "/example_evaluation.txt")
cat("# columns 'breeder' and 'ind' are compulsory\n",
    file=f, append=FALSE)
cat("# the 'breeder' column should contain players' names which already exist in the game\n",
    file=f, append=TRUE)
cat("# the 'ind' column should contain plant material identifiers which already exist in the game\n",
    file=f, append=TRUE)
cat("# use write.table(x=dat, file=\"eval.txt\", quote=FALSE, sep=\"\\t\", row.names=FALSE, col.names=TRUE)\n", 
    file=f, append=TRUE)
